...
namespace Parallax
{
	...
	class ExampleParallax : public entry::AppI
	{
	public:
		ExampleParallax(const char* _name, const char* _description, const char* _url)
		...
		{
			std::string prefix("../50-parallax/");
			m_program = compileSingleGraphicsProgram(prefix, "vs_parallax", "fs_parallax");
		}
		...
		void init(int32_t _argc, const char* const* _argv, uint32_t _width, uint32_t _height) override
		{
			...
			m_vbh = bgfx::createVertexBuffer(bgfx::makeRef(s_cubeVertices, sizeof(s_cubeVertices)), PosTangentBitangentTexcoordVertex::ms_layout);
		...
		bool update() override
		{
			...
			float params[4] = { float(m_shading_type), float(m_show_diffuse_texture), float(m_parallax_scale), float(m_num_steps) };
...
ENTRY_IMPLEMENT_MAIN(
	Parallax::ExampleParallax
	, "50-Parallax Occlusion Mapping"

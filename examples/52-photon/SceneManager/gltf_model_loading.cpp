...
namespace Dolphin
{
	...
	Mesh processPrimitive(tinygltf::Model& gltf_model, const tinygltf::Primitive& primitive)
	{
		...
		bgfx::VertexLayout decls[BX_COUNTOF(ATTRIBUTE_NAMES)];
		...
		if (node.mesh != -1)
		{
			...
			for (const tinygltf::Primitive& primitive : mesh.primitives)
			{
				if (primitive.material != -1)
				{
					...
					boundingBox.m_min = glm::vec3{ transform * glm::vec4{ boundingBox.m_min, 1.0f } };
					boundingBox.m_max = glm::vec3{ transform * glm::vec4{ boundingBox.m_max, 1.0f } };
					const std::pair<PBRMaterial, TransparencyMode>& materialModePair = materials_list[primitive.material];

					output_model.boundingBox = { glm::min(output_model.boundingBox.m_min, boundingBox.m_min), glm::max(output_model.boundingBox.m_max, boundingBox.m_max) };

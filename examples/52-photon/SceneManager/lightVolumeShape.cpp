...
namespace Dolphin
{
	const float lightVolumeT = (1.0f + bx::sqrt(5.0f)) / 2.0f;

	std::vector<glm::vec3> basicIcosahedronPositions = {
		{-1.0f,  lightVolumeT,  0.0f},
		{ 1.0,  lightVolumeT,  0.0f},
		{-1.0f, -lightVolumeT,  0.0f},
		{ 1.0, -lightVolumeT,  0.0f},

		{ 0.0f, -1.0f,  lightVolumeT},
		{ 0.0f,  1.0,  lightVolumeT},
		{ 0.0f, -1.0f, -lightVolumeT},
		{ 0.0f,  1.0, -lightVolumeT},

		{ lightVolumeT, 0.0f, -1.0f},
		{ lightVolumeT, 0.0f,  1.0},
		{-lightVolumeT, 0.0f, -1.0f},
		{-lightVolumeT, 0.0f,  1.0},
	};
	...
	lightVolumeShape::lightVolumeShape(uint8_t _detail)
		:detail{ _detail },
		...
		indices{ basicIcosahedronIndices }
	{
		...
		for (uint8_t subdivisionLevel = 0; subdivisionLevel < detail; ++subdivisionLevel) {
			...
			for (size_t i = 0; i < numIndices; i += 3)	//for each face
			{
	...
	uint16_t lightVolumeShape::getOrCreateMidPoint(uint16_t first, uint16_t second)
	{

...
vec2 CalculateDensity(vec3 rayStart, vec3 rayDir)
{
...
void GetLocalAndToTopAtmosphereDensity(vec3 position, vec3 planetCenter, vec3 lightDir, out vec2 localDensity, out vec2 densityToAtmosphereTop)
{
	...
	densityToAtmosphereTop = CalculateDensity(rayStart, rayDir);
}

void CalculateLocalInscattering(vec2 localDensity, vec2 densityPA, vec2 densityCP, out vec3 localInscatterR, out vec3 localInscatterM)
{
...
float Sun(float cosAngle)
{
	...
	float sun = pow(1 - g, 2.0) / (4.0 * M_PI * pow(1.0 + g2 - 2.0 * g * cosAngle, 1.5));
...
vec4 IntegrateWholeInscattering(vec3 rayStart, vec3 rayDir, float rayLength, vec3 planetCenter, float distanceScale, vec3 lightDir, float sampleCount, out vec4 extinction)
{
	...
	GetLocalAndToTopAtmosphereDensity(rayStart, planetCenter, lightDir, prevLocalDensity, densityPA);
	CalculateLocalInscattering(prevLocalDensity, densityPA, densityCP, prevLocalInscatterR, prevLocalInscatterM);

	for(float s = 1.0; s < sampleCount; s += 1.0)
	{
		vec3 p = rayStart + step * s;

		GetLocalAndToTopAtmosphereDensity(p, planetCenter, lightDir, localDensity, densityPA);
		densityCP += (localDensity + prevLocalDensity) * (stepSize / 2.0);

		prevLocalDensity = localDensity;

		vec3 localInscatterR, localInscatterM;
		CalculateLocalInscattering(localDensity, densityPA, densityCP, localInscatterR, localInscatterM);
...
void main()
{
	...
	vec4 inscattering = IntegrateWholeInscattering(rayStart, rayDir, rayLength, planetCenter, DistanceScale, lightDir, 16, extinction);

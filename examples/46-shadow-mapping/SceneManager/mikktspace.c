...
tbool genTangSpace(const SMikkTSpaceContext * pContext, const float fAngularThreshold)
{
	...
	for (f=0; f<iNrFaces; f++)
	{
		...
		for (i=0; i<verts; i++)
		{
			...
			float tang[3];
			tang[0] = pTSpace->vOs.x;
			tang[1] = pTSpace->vOs.y;
			tang[2] = pTSpace->vOs.z;

			//float bitang[] = {pTSpace->vOt.x, pTSpace->vOt.y, pTSpace->vOt.z};
			float bitang[3];
			bitang[0] = pTSpace->vOt.x;
			bitang[1] = pTSpace->vOt.y;
			bitang[2] = pTSpace->vOt.z;
		...
		else
		{
...

			{
				...
				else
				{
					...
					const float local_distSQ_02 = LengthSquared(vsub(P2,P0));
					const float local_distSQ_13 = LengthSquared(vsub(P3,P1));

					bQuadDiagIs_02 = local_distSQ_13 < local_distSQ_02 ? TFALSE : TTRUE;
				}
		...
		for (i=0; i<pGroup->iNrFaces; i++)	// triangles
		{
			...
			if (!bFound)
			{
				...
				if (pIndices==NULL)
				{
					// clean up and return false
					//int s=0;
					for (int ss=0; ss<iUniqueSubGroups; ss++)
						free(pUniSubGroups[ss].pTriMembers);
					free(pUniSubGroups);
					free(pTmpMembers);
...
static void BuildNeighborsFast(STriInfo pTriInfos[], SEdge * pEdges, const int piTriListIn[], const int iNrTrianglesIn)
{
	...
	for (i=0; i<iEntries; i++)
	{
		...
		if (bUnassigned_A)
		{
			...
			{
				int tt = pEdges[j].f;
				pTriInfos[f].FaceNeighbors[edgenum_A] = tt;
				//assert(pTriInfos[t].FaceNeighbors[edgenum_B]==-1);
				pTriInfos[tt].FaceNeighbors[edgenum_B] = f;
			}
		}
	}
}
...
static void DegenPrologue(STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn, const int iTotTris)
{
	...
	while (t<iNrTrianglesIn && bStillFindingGoodOnes)
	{
		...
		else
		{
			...
			{
				const tbool bLocalIsGood = (pTriInfos[iNextGoodTriangleSearchIndex].iFlag&MARK_DEGENERATE)==0 ? TTRUE : TFALSE;
				if (bLocalIsGood) bJustADegenerate=TFALSE;

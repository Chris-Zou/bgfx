...
#define u_lightIntensity u_directionalLightParams[0].w
...
{
	float NoL =  clampDot(normal, lightDir);
	float N_offset_scale = sqrt(1.0 - NoL * NoL);
	float L_offset_scale = N_offset_scale / NoL;
...
float sampleLightDepth(uint cascadeIdx, vec2 texCoords, vec2 diskRotation, float lightDepth, uint i, uint j)
{
	vec2 diskOffset = vec2(diskRotation.x * u_samplingDisk[i][j] - diskRotation.y * u_samplingDisk[i][j+1],
	...
	if(cascadeIdx == 0)
	{
		sampleLightDepth = texture2D(s_shadowMap_1, uv).r;
	}
	else if(cascadeIdx == 1)
	{
		sampleLightDepth = texture2D(s_shadowMap_2, uv).r;
	}
	else if(cascadeIdx == 2)
	{
		sampleLightDepth = texture2D(s_shadowMap_3, uv).r;
	}
	else
	{
		sampleLightDepth = texture2D(s_shadowMap_4, uv).r;
	}

	return step(lightDepth, sampleLightDepth);
}
...
void main()
{
	...
	vec3 light = shadowVisibility * u_lightIntensity * u_lightColor * selfOcclusion;

	vec3 color = 0.1 * diffuseColor(baseColor.xyz, metallic) + (diffuseColor(baseColor.xyz, metallic) + PI * specular(-u_lightDir, viewDir, normal, baseColor.xyz, roughness, metallic)) * light;

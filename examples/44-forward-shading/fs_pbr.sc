...
void main()
{
	...
	for(uint i = 0; i < numLights; ++i)
	{
		...
		color += (diffuseColor(baseColor.xyz, metallic) + PI * specular(lightDir, viewDir, normal, baseColor.xyz, roughness, metallic)) * light;
	}
	gl_FragColor = vec4(color + emissive, baseColor.w);
}

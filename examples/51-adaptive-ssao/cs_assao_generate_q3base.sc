...
void GenerateSSAOShadowsInternal( out float outShadowTerm, out vec4 outEdges, out float outWeight, const vec2 SVPos)
{
	...
	const uint numberOfTaps = SSAO_ADAPTIVE_TAP_BASE_COUNT;
	...
	{
		SSAOTap(obscuranceSum, weightSum, int(i), rotScale, pixCenterPos, negViewspaceDir, pixelNormal, normalizedScreenPos, mipOffset, falloffCalcMulSq, 1.0, normXY, normXYLength);
	}
